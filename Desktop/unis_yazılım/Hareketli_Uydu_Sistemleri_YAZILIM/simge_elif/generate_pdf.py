...
def _register_turkish_font():
    """Windows'taki Arial fontunu kaydederek Turkce karakter destegi saglar."""
    try:
...
def main() -> None:
    source = Path(__file__).with_name("Mergen_Proje_Raporu.md")
    target = Path(__file__).with_name("Mergen_Proje_Raporu.pdf")
    ...
    styles.add(ParagraphStyle(
        "CodeBlock", fontName="Courier" if not has_turkish else "Arial",
    ...
    def flush_code():
        nonlocal code_lines
        if code_lines:
            ctext = "<br/>".join(code_lines)
            story.append(Paragraph(ctext, styles["CodeBlock"]))
            code_lines = []
